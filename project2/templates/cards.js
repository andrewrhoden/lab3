...
var setAudio = function() {
    ...
    if(clicks > 23){
        var audioElement = document.createElement('audio');
        audioElement.setAttribute('src', 'http://www.shockwave-sound.com/sound-effects/laugh-sounds/Silly%20High%20Pitched%20Laugh.mp3');
        audioElement.load()
        audioElement.play();
...
".b{padding-top:0px; background-image: url(/static/cardpic.png);background-size:100px 100px;"+P("transition:1s")+"}";
...
function F(t){
    ...
    if(!l) {
        var audioElement = document.createElement('audio');
        audioElement.setAttribute('src', 'http://upload.wikimedia.org/wikipedia/en/1/17/01_Celebrate_%28feat._Pharrell_Williams%29.ogg');
        audioElement.load()
        audioElement.play();
